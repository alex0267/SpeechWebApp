...
const Button = ({onClick, title, isHidden=false, extraClass=''}) => {
    const hiddenClasses = isHidden ? " pointer-events-none opacity-20 " : "";
      return <button
        onClick={onClick}
               className={"button w-1/2 flex items-center justify-center rounded-full bg-black text-white px-6 py-2.5 m-2 ".concat(hiddenClasses).concat(extraClass)}>
        {title}
      </button>
};
